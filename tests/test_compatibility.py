...
class Compatibility(unittest.TestCase):
 ...
 def test_uploaded_image_reaches_home_and_case_without_code_changes(self):
  target=self.root/'dist/images/karbon-map.webp';payload={'version':1,'assets':[{'path':'images/uploads/new-work.webp','data':base64.b64encode(target.read_bytes()).decode()}],'items':[]};m=next(x for x in json.loads((self.root/'data/media.json').read_text())['items'] if x['id']=='karbonkreds-website.jpg');m['src']='images/uploads/new-work.webp';m['alt']='Updated carbon platform presentation';payload['items']=[m];p=self.root/'update.json';p.write_text(json.dumps(payload));imp.apply(p,self.root);before=(self.root/'data/media.json').read_bytes();self.build();self.assertEqual(before,(self.root/'data/media.json').read_bytes())
